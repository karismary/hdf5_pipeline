...
def show_tab_rename() -> None:
    ...
    if st.session_state.get("locked"):
        with st.container(key = "tabre_tip", border = True, horizontal_alignment = "center"):
            st.subheader("该界面已被锁定")
            if st.button("取消锁定", key = "tabre_unlock"):
                st.session_state["locked"] = False
                st.rerun()
    else:
        st.subheader("文件整理与重命名")
        with st.container(key = "tabre_container", border = True):

            colre1, colre2 = st.columns([4,1])
            with colre1:
                src_dir = st.text_input("源目录",
                                        key = "tabre_src_dir_ti",
                                        placeholder = "未选择 - hdf5源目录（含子目录的原始 HDF5 文件夹）",
                                        label_visibility = "collapsed")
                dst_dir = st.text_input("输出总目录",
                                        key = "tabre_dst_dir_ti",
                                        placeholder = "未选择 - 输出总目录（将hdf5文件重命名汇总到统一文件夹下）",
                                        label_visibility = "collapsed")
            
            with colre2:
                st.button("📂浏览", key = "tabre_srcdir_bt", width = "stretch", on_click = folder_callback, args = ("tabre_src_dir_ti", ))
                st.button("📂浏览", key = "tabre_dstdir_bt", width = "stretch", on_click = folder_callback, args = ("tabre_dst_dir_ti", ))
            ...
            if st.button("锁定界面", key = "tabre_lock", width = "stretch"):
